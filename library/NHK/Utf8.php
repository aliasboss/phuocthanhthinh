...
class NHK_Utf8 {
	...
	function convert($string, $encoding = '', $destination = 'UTF-8') {
		if ($encoding == '') $encoding = D_Utf8::detect($string);
	...
	function strwidth($str) {
		...
		return D_Utf8::strlen($str) - $null + $double;
	}
	...
	function strimwidth($str, $start, $width, $trimmarker = '') {
		...
		$str   = D_Utf8::substr($str, $start);
		$width = $width - D_Utf8::strwidth($trimmarker);
		...
		for ($i = 0; $i < strlen($str); $i++)
		{
			...
			if ($count > $width) {
				return D_Utf8::substr($str, 0, $c - 1) . $trimmarker;
			}
		}
	}
	...
	function strrpos($haystack, $needle) {
		...
		if ($pos === false)
			...
			return D_Utf8::strlen(substr($haystack, 0, $pos));
	}
	...
	function strpos($haystack, $needle, $offset = 0) {
		...
		while (!isset($length) || $length < $offset)
		{
			...
			$length = D_Utf8::strlen(substr($haystack, 0, $pos));
	...
	function send_mail($to, $subject, $message , $additional_headers = '', $additional_parameter = '') {
		$subject = D_Utf8::encode_mimeheader($subject);
	...
	function is_half($instr) {
	    ...
	    for ($ofst = 0; $ofst < $len; $ofst++) {
	        $ch = mb_substr($instr, $ofst, 1);
	        if (mb_ereg('^[ｱ-ﾝﾞﾟｦｰ]+$', $ch)) {
	            $flag = TRUE;
	        } else if (mb_ereg('^[a-zA-Z0-9]+$', $ch)) {
	            $flag = TRUE;
	        } else if (strrpos($ch, $symbol)) {
	            $flag = TRUE;
	        } else {
	            $flag = false;
	        }
	...
	function is_full_width($text){
		mb_regex_encoding("SHIFT_JIS"); 
		return mb_ereg("^[ア-ンア-ヶ]+$", $text);
		return mb_ereg("^[]+$", $text);
		return mb_ereg('^([\x82][\x9f-\xed\xf0\xf1])*$', $text);
	}
	
	function is_full_kana($text){
		return mb_ereg("^[ア-ンア-ヶ]+$", $text);
	}
	
	static function is_katakana($text)
	{
	...
	function is_hiragana($text){
		return preg_match("/^[ぁ-ん]+$/u", $text); 
        //echo "hiragana";  
	}
	
	function is_half_kana($str){
		$pattern = '/[^\wぁ-ゔァ-ヺー\x{4E00}-\x{9FAF}_\-]+/u';
		return preg_match($pattern, $str);
	}
	
	function ss($str){
		            $str = mb_convert_kana($str, "asKHV");
