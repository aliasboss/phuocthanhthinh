...
class Admin_NewsController extends NHK_AdminController {
...
//    exit();
    ...
    function deleteallAction() {
        ...
        if ($this->_request->isPost()) {
            ...
            $this->_model->execute("truncate news");
            $reponse['status'] = 'success';
    ...
    function phuocthanhthinhAction() {
        if ($this->_request->isPost()) {
            ...
            $this->redirect(NHK_URL::getRouteAdmin('news'));
        }
    }

    public function thongBaoAction() {
        $this->view->menu = 'thongbao';
        if ($this->_request->isPost()) {
            $params = $this->_request->getParams();
            $data = $params['data'];
            //$data['title_slug'] = NHK_Utils::generateUrlSlug($data['title']);
                $data['deleted_at'] = date('Y-m-d H:i:s', strtotime($data['deleted_at']));
              
                $this->_model->update($data, "id={$params['id']}");
                //$this->noView();
                $this->setView('index/index.phtml');
                $this->view->notifacation =  "Cập Nhật Thông Báo Thành Công"; 
                
        } else {
            $data = $this->_model->get(1)->toArray();
            $this->view->params = $data;
        }
    }
